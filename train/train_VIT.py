...
from model.model_VIT import VisionTransformer, get_config
...
config = get_config()
COMMON_DIR = ('transformer/24-6_12_36/' + str(NUM_CLASSES) + '_' +
            str(index+1) + 'th/')
...
################################################################################
# Define the model.
net = VisionTransformer(config, num_classes=42, vis=True)
...
for epoch in range(num_epochs):
    ...
    for (samples, labels) in tqdm(train_data_loader):
        ...
        if labelsV.ndim == 1 or predict_label[0].ndim == 1:
            ...
            predict_label[1] = predict_label[1].unsqueeze(0)
