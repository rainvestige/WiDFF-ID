...
COMMON_DIR = ('orientation2/24-6_12_36/' + str(NUM_CLASSES) + '_' +
            str(index+1) + 'th/')
MODEL_SAVEDIR = '../weights/' + COMMON_DIR
MODEL_SAVENAME = (MODEL_SAVEDIR + 'aug_pca' + str(tx_rx_path) + '_percent' +
                 str(percent) + '.pkl')
...
################################################################################
# change the num_classes from 30 to 13
net = CSIDenseNet(growth_rate=24, block_config=(6, 12, 36),
...
for epoch in range(num_epochs):
    ...
    for (samples, labels) in tqdm(train_data_loader):
        ...
        if labelsV.ndim == 1 or predict_label.ndim == 1:
